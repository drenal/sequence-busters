...
class LenardModel6(ModelBase):
    def __init__(self, in_features: int):
        ...
        #self.L1 = nn.Conv1d(in_channels=64, out_channels=128, kernel_size=9, padding=4, padding_mode='circular')
        #self.N1 = nn.ReLU()
        ...
        self.L2 = nn.LSTM(input_size=self.input_dim, hidden_size=self.hidden_dim, num_layers=self.n_layers, batch_first=True, bidirectional=False)
        self.N2 = nn.ReLU()

        # FC to Q8 and Q3
        self.ss8 = nn.Linear(self.hidden_dim, 8)
        self.ss3 = nn.Linear(self.hidden_dim, 3)
    ...
    def forward(self, x: torch.tensor, mask: torch.tensor) -> torch.tensor:
        ...
        hidden_state = torch.randn(self.n_layers, batch_size, self.hidden_dim).to("cuda")
        cell_state = torch.randn(self.n_layers, batch_size, self.hidden_dim).to("cuda")
